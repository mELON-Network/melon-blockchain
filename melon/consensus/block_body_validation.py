...
from melon.consensus.block_root_validation import validate_block_merkle_roots
from melon.full_node.mempool_check_conditions import mempool_check_conditions_dict
...
from melon.types.condition_opcodes import ConditionOpcode
...
from melon.util import cached_bls
from melon.util.condition_tools import (
    pkm_pairs_for_conditions_dict,
    coin_announcements_names_for_npc,
    puzzle_announcements_names_for_npc,
)
from melon.util.errors import Err
from melon.util.generator_tools import (
    additions_for_npc,
...
async def validate_block_body(
    ...
    if height > 0:
        ...
        elonwallet_coin = create_elonwallet_coin(
            ...
            calculate_base_elonwallet_reward(prev_transaction_block.height),
            constants.GENESIS_CHALLENGE,
        )
        # Adds the previous block
        expected_reward_coins.add(pool_coin)
        expected_reward_coins.add(farmer_coin)
        expected_reward_coins.add(elonwallet_coin)
        ...
        if prev_transaction_block.height > 0:
            ...
            while not curr_b.is_transaction_block:
                ...
                expected_reward_coins.add(
                        create_elonwallet_coin(
                        curr_b.height,
                        constants.GENESIS_PRE_FARM_ELONWALLET_PUZZLE_HASH,
                        calculate_base_elonwallet_reward(curr_b.height),
                        constants.GENESIS_CHALLENGE,
                    )
                )
    ...
    # 18. Check that the fee amount + farmer reward < maximum coin amount
    if fees + calculate_base_elonwallet_reward(height) > constants.MAX_COIN_AMOUNT:
        return Err.COIN_AMOUNT_EXCEEDS_MAXIMUM, None
