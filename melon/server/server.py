...
from melon.server.introducer_peers import IntroducerPeers
from melon.server.outbound_message import Message, NodeType
from melon.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from melon.util.errors import Err, ProtocolError
from melon.util.ints import uint16
from melon.util.network import is_localhost, is_in_network
...
class melonServer:
    def __init__(
        ...
        melon_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSmelonConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSmelonConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.melon_ca_crt_path, self.melon_ca_key_path = melon_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSmelonConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.melon_ca_crt_path, self.melon_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSmelonConnection] = None
        try:
            connection = WSmelonConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSmelonConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSmelonConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSmelonConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSmelonConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSmelonConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSmelonConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSmelonConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSmelonConnection, trusted_peers: Dict) -> bool:
