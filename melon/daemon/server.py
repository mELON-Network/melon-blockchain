...
from melon.cmds.init_funcs import melon_init
from melon.daemon.windows_signal import kill
from melon.server.server import ssl_context_for_root, ssl_context_for_server
from melon.ssl.create_ssl import get_mozilla_ca_crt
from melon.util.melon_logging import initialize_logging
...
from melon.util.service_groups import validate_service
from melon.util.setproctitle import setproctitle
from melon.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting melon.")
    quit()
...
service_plotter = "melon plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "melon": "melon",
        "melon_wallet": "start_wallet",
        "melon_full_node": "start_full_node",
        "melon_harvester": "start_harvester",
        "melon_farmer": "start_farmer",
        "melon_introducer": "start_introducer",
        "melon_timelord": "start_timelord",
        "melon_timelord_launcher": "timelord_launcher",
        "melon_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("melon daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered melon_ROOT
    os.environ["melon_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with melon_ROOT: {os.environ['melon_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    melon_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("melon_daemon")
