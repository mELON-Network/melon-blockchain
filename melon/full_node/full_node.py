...
import melon.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from melon.consensus.block_creation import unfinished_block_to_full_block
from melon.consensus.block_record import BlockRecord
from melon.consensus.blockchain import Blockchain, ReceiveBlockResult
from melon.consensus.constants import ConsensusConstants
from melon.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from melon.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from melon.consensus.multiprocess_validation import PreValidationResult
from melon.consensus.pot_iterations import calculate_sp_iters
from melon.full_node.block_store import BlockStore
from melon.full_node.bundle_tools import detect_potential_template_generator
from melon.full_node.coin_store import CoinStore
from melon.full_node.full_node_store import FullNodeStore
from melon.full_node.mempool_manager import MempoolManager
from melon.full_node.signage_point import SignagePoint
from melon.full_node.sync_store import SyncStore
from melon.full_node.weight_proof import WeightProofHandler
from melon.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from melon.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from melon.protocols.protocol_message_types import ProtocolMessageTypes
from melon.server.node_discovery import FullNodePeers
from melon.server.outbound_message import Message, NodeType, make_msg
from melon.server.server import melonServer
from melon.types.blockchain_format.classgroup import ClassgroupElement
from melon.types.blockchain_format.pool_target import PoolTarget
from melon.types.blockchain_format.sized_bytes import bytes32
from melon.types.blockchain_format.sub_epoch_summary import SubEpochSummary
from melon.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
from melon.types.end_of_slot_bundle import EndOfSubSlotBundle
from melon.types.full_block import FullBlock
from melon.types.header_block import HeaderBlock
from melon.types.mempool_inclusion_status import MempoolInclusionStatus
from melon.types.spend_bundle import SpendBundle
from melon.types.unfinished_block import UnfinishedBlock
from melon.util.bech32m import encode_puzzle_hash
from melon.util.db_wrapper import DBWrapper
from melon.util.errors import ConsensusError, Err
from melon.util.ints import uint8, uint32, uint64, uint128
from melon.util.path import mkdir, path_from_root
from melon.util.safe_cancel_task import cancel_task_safe
from melon.util.profiler import profile_task
...
class FullNode:
    ...
    def set_server(self, server: melonServer):
        self.server = server
        dns_servers = []
        try:
        ...
        elif self.config["port"] == 4420:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.melon-network.de")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSmelonConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSmelonConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSmelonConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSmelonConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSmelonConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSmelonConnection):
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSmelonConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSmelonConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSmelonConnection],
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'melon')}, "
                f"validation time: {validation_time}, "
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSmelonConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSmelonConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSmelonConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSmelonConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSmelonConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
