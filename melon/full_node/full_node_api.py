...
import melon.server.ws_connection as ws
from melon.consensus.block_creation import create_unfinished_block
from melon.consensus.block_record import BlockRecord
from melon.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from melon.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from melon.full_node.full_node import FullNode
from melon.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from melon.full_node.signage_point import SignagePoint
from melon.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from melon.protocols.full_node_protocol import RejectBlock, RejectBlocks
from melon.protocols.protocol_message_types import ProtocolMessageTypes
from melon.protocols.wallet_protocol import PuzzleSolutionResponse, RejectHeaderBlocks, RejectHeaderRequest
from melon.server.outbound_message import Message, make_msg
from melon.types.blockchain_format.coin import Coin, hash_coin_list
...
from melon.types.coin_record import CoinRecord
from melon.types.end_of_slot_bundle import EndOfSubSlotBundle
from melon.types.full_block import FullBlock
from melon.types.generator_types import BlockGenerator
from melon.types.mempool_inclusion_status import MempoolInclusionStatus
from melon.types.mempool_item import MempoolItem
from melon.types.peer_info import PeerInfo
from melon.types.unfinished_block import UnfinishedBlock
from melon.util.api_decorators import api_request, peer_required, bytes_required, execute_task
from melon.util.generator_tools import get_block_header
from melon.util.hash import std_hash
from melon.util.ints import uint8, uint32, uint64, uint128
from melon.util.merkle_set import MerkleSet
...
class FullNodeAPI:
    ...
    @peer_required
    @api_request
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSmelonConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSmelonConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
        """
        ...
        async with self.full_node.timelord_lock:
            ...
            if prev_b is None:
                ...
                elonwallet_ph = self.full_node.constants.GENESIS_PRE_FARM_ELONWALLET_PUZZLE_HASH
            else:
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSmelonConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSmelonConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
