...
from melon.cmds.units import units
...
from melon.util.misc import format_bytes
from melon.util.misc import format_minutes
from melon.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total melon farmed: {amounts['farmed_amount'] / units['melon']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['melon']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['elonwallet_reward_amount'] + amounts['pool_reward_amount']) / units['melon']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'melon start wallet' and 'melon wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'melon wallet show'")
    else:
        print("Note: log into your key using 'melon wallet show' to see rewards for each key")
