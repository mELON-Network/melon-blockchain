...
from melon.cmds.init_funcs import check_keys
from melon.consensus.block_rewards import calculate_base_farmer_reward, calculate_base_elonwallet_reward
from melon.pools.pool_wallet import PoolWallet
from melon.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from melon.protocols.protocol_message_types import ProtocolMessageTypes
from melon.server.outbound_message import NodeType, make_msg
from melon.simulator.simulator_protocol import FarmNewBlockProtocol
from melon.types.blockchain_format.coin import Coin
from melon.types.blockchain_format.sized_bytes import bytes32
from melon.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from melon.util.byte_types import hexstr_to_bytes
from melon.util.ints import uint32, uint64
from melon.util.keychain import bytes_to_mnemonic, generate_mnemonic
from melon.util.path import path_from_root
from melon.util.ws_message import WsRpcMessage, create_payload_dict
from melon.wallet.cc_wallet.cc_wallet import CCWallet
from melon.wallet.derive_keys import master_sk_to_singleton_owner_sk
from melon.wallet.rl_wallet.rl_wallet import RLWallet
from melon.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
from melon.wallet.did_wallet.did_wallet import DIDWallet
from melon.wallet.trade_record import TradeRecord
from melon.wallet.transaction_record import TransactionRecord
from melon.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from melon.wallet.util.trade_utils import trade_record_to_dict
from melon.wallet.util.transaction_type import TransactionType
from melon.wallet.util.wallet_types import WalletType
from melon.wallet.wallet_info import WalletInfo
from melon.wallet.wallet_node import WalletNode
from melon.util.config import load_config
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "melon_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "melon_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("melon_target_address")
        pool_target = config["pool"].get("melon_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from melon.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def get_farmed_amount(self, request):
        ...
        for record in tx_records:
            ...
            if( uint64(calculate_base_elonwallet_reward(height)) != uint64(record.amount) ):
        ...
        return {
            ...
            "elonwallet_reward_amount": elonwallet_reward_amount,
