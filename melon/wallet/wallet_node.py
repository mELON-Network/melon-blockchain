...
from melon.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from melon.protocols import wallet_protocol
from melon.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from melon.protocols.protocol_message_types import ProtocolMessageTypes
from melon.protocols.wallet_protocol import (
...
from melon.util.errors import Err, ValidationError
from melon.util.ints import uint32, uint128
from melon.util.keychain import Keychain
from melon.util.lru_cache import LRUCache
from melon.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from melon.wallet.settings.settings_objects import BackupInitialized
...
from melon.wallet.wallet_blockchain import ReceiveBlockResult
from melon.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'melon keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSmelonConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSmelonConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSmelonConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSmelonConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSmelonConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSmelonConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
