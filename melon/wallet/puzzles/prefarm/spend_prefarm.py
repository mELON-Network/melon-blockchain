...
from melon.rpc.full_node_rpc_client import FullNodeRpcClient
...
from melon.util.condition_tools import parse_sexp_to_conditions
from melon.util.config import load_config
from melon.util.default_root import DEFAULT_ROOT_PATH
from melon.util.ints import uint32, uint16
...
async def main() -> None:
    ...
    try:
        elonwallet_prefarm = (await client.get_block_record_by_height(1)).reward_claims_incorporated[2]
        ...
        elonwallet_amounts = int(calculate_base_elonwallet_reward(uint32(0)) / 2)
        print(farmer_prefarm.amount, farmer_amounts)
        assert farmer_amounts == farmer_prefarm.amount // 2
        assert pool_amounts == pool_prefarm.amount // 2
        assert elonwallet_amounts == elonwallet_prefarm.amount // 2
        address1 = "melon135ztetmkx6wjw0zv305mg03efwx09575jl7fufy8cqsq52v2s9zq3yc0jf"  # Key 1
        address2 = "melon135ztetmkx6wjw0zv305mg03efwx09575jl7fufy8cqsq52v2s9zq3yc0jf"  # Key 2
        address2 = "melon135ztetmkx6wjw0zv305mg03efwx09575jl7fufy8cqsq52v2s9zq3yc0jf"  # Key 2
        ...
        p_elonwallet_2 = Program.to(
            binutils.assemble(f"(q . ((51 0x{ph1.hex()} {elonwallet_amounts}) (51 0x{ph2.hex()} {elonwallet_amounts})))")
        )
        ...
        sb_elonwallet = SpendBundle([CoinSpend(elonwallet_prefarm, p_elonwallet_2, p_solution)], G2Element())
        ...
        # res = await client.push_tx(sb_elonwallet)
        ...
        uc = await client.get_coin_records_by_puzzle_hash(elonwallet_prefarm.puzzle_hash, True)
        print(up)
        print(uf)
        print(uc)
    finally:
