...
import melonEnvironment from '../util/melonEnvironment';
import melonConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!melonEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    melonConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!melonConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !melonConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
          ...
          const choice = dialog.showMessageBoxSync({
            ...
            buttons: [
              i18n._(/* i18n */ { id: 'No !' }),
              i18n._(/* i18n */ { id: 'Hell yeah' }),
            ],
            title: i18n._(/* i18n */ { id: 'Confirm' }),
            message: i18n._(
              /* i18n */ {
                id: 'Are you sure you want to quit? GUI Plotting and farming mELON will stop.',
              },
            ),
          });
          if (choice == 0) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (melonConfig.manageDaemonLifetime()) {
        melonEnvironment.startmelonDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      {
        label: i18n._(/* i18n */ { id: 'mELON Links' }),
        role: 'mELON Links',
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'mELON Anthem' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Do not click' }),
            click: () => {
              openExternal(
                'https://www.cnet.com/a/img/gYqc1tZjVGRoIKdg_MqBeZfFltU=/940x0/2019/12/15/71f4408f-b4c7-4ecd-bfea-6f6151e9a3d9/gettyimages-1130598318.jpg',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/mELON-Network/melon-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/mELON-Network/melon-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          {
            label: i18n._(/* i18n */ { id: '100% approved' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Post ur fav Elon Meme on discord' }),
            click: () => {
              openExternal('https://discord.gg/Q8eVStYegg');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow the boss on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/elonmusk');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // mELON Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'melon' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About mELON Blockchain' }),
            click: () => {
              openAbout();
            },
          },
