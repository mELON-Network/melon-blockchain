...
export default function Block() {
  ...
  const poolReward = mojo_to_melon(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_melon(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const melonFees = blockRecord.fees
    ? mojo_to_melon(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/melon/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/melon/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: melonFees ? `${melonFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the mELON Blockchain
            </Trans>
          </BlockTitle>
        }
        action={
