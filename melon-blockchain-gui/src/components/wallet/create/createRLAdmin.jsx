...
import { AlertDialog } from '@melon/core';
import {
  Typography,
  Button,
  Box,
...
import { melon_to_mojo } from '../../../util/melon';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      melonper_input.value === '' ||
      Number(melonper_input.value) === 0 ||
      !Number(melonper_input.value) ||
      isNaN(Number(melonper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const melonper = melon_to_mojo(melonper_input.value);
    const melonper_value = Number.parseInt(Number(melonper));
    const userpubkey = userpubkey_input.value;
    const amount = melon_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = melon_to_mojo(fee_input.value);
