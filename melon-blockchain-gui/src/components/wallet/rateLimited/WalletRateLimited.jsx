...
import { AlertDialog, Card, Flex } from '@melon/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const melonper_input = ip_parsed.limit;
    ...
    const melonper_value = Number.parseInt(Number(melonper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (melon per interval):{' '}
                  {mojo_to_melon_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = melon_to_mojo(fee_input.value);
