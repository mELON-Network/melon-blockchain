...
const getmelonVersion = () => {
  let version = null;
  const exePath = getExecutablePath('melon');
  // first see if we can get a melon exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or melon is on the path
    try {
      version = child_process
