...
global.daemon_rpc_ws = `wss://${self_hostname}:44420`;
...
function loadConfig(net) {
  try {
    // check if melon_ROOT is set. it overrides 'net'
    const config_root_dir =
      'melon_ROOT' in process.env
        ? process.env.melon_ROOT
        : path.join(os.homedir(), '.melon', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 44420); // jshint ignore:line
