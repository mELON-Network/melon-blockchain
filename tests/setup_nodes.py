...
from melon.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from melon.full_node.full_node_api import FullNodeAPI
from melon.server.start_farmer import service_kwargs_for_farmer
from melon.server.start_full_node import service_kwargs_for_full_node
from melon.server.start_harvester import service_kwargs_for_harvester
from melon.server.start_introducer import service_kwargs_for_introducer
from melon.server.start_service import Service
from melon.server.start_timelord import service_kwargs_for_timelord
from melon.server.start_wallet import service_kwargs_for_wallet
from melon.simulator.start_simulator import service_kwargs_for_full_node_simulator
from melon.timelord.timelord_launcher import kill_processes, spawn_process
...
async def setup_farmer(
    ...
    config["melon_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "melon")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["melon_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "melon")
